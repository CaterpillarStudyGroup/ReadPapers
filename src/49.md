...
||||
|--|--|--|
|当前方法：滑窗策略 |![](./assets/49-手图1.png)|两个CLip分别推断，在overlap处fusion.  这会导致两个clip的明显不连续，和overlap处的artifacts.   |
|本文方法|![](./assets/49-手图2.png)| 直接用上一帧的尾帧在为这一帧的首帧。优势：1.可生成指定首帧的视频。 2.可生成一致的长视频。    |  
