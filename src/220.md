...
**核心思想**：每个高斯球不再绑定单一骨骼，而是通过 **soft assignment** 软分配到多个 part 的混合分布上，形成 Mixture-of-Experts 式的绑定。

**和 LBS 的对比**：

| | LBS | PartMM |
|---|---|---|
| 绑定方式 | 固定骨骼权重 + 线性组合 | soft assignment → 混合 part 分布 |
| 变形计算 | \\(v' = \Sigma w_i \cdot (R_i v + t_i)\\) | 最近邻匹配变形后位置 |
| 关节非线性形变 | 差（面条臂伪影） | 好（通过匹配学习非线性变形） |

**流程**：
1. 提取 skeletal graph（骨架图），分解为多个 parts
2. 每个 part 维护一个变形模型
3. 高斯球通过 soft assignment 绑定到多个 parts
4. 运动时通过**最近邻搜索**在 part 的混合分布中找到变形后的位置
