...
$$\mathcal{G}_f^{(\cdot)} = \mathcal{G}_c^{(\cdot)} + D_{\text{MLP}}^{(\cdot)}(f), \quad (\cdot) \in \{\mu, c, \sigma, q, s\}$$

**设计选择**：使用**各向同性高斯**（而非各向异性），牺牲少量渲染质量以换取对新姿态更好的泛化能力（following RigGS）。

**训练损失**：光度损失（L1 + D-SSIM）：

$$\mathcal{L}_{\text{photo}} = \sum_{f=1}^{F} \left[(1-\lambda)\|I_f - \hat{I}_f\|_1 + \lambda(1 - \text{D-SSIM}(I_f, \hat{I}_f))\right]$$

### 3.2 局部刚性正则化（ARAP）

为了满足后续 SSDR 对时序一致性的要求，引入 ARAP 约束 + 距离保持约束：

$$\mathcal{L}_{\text{rigid}} = \sum_{(i,j)\in K} \left[ \underbrace{\|(x_f^i - x_f^j) - R_i(x_c^i - x_c^j)\|_2^2}_{\text{ARAP 项}} + \underbrace{(\|x_f^i - x_f^j\|_2 - \|x_c^i - x_c^j\|_2)^2}_{\text{距离保持项}} \right]$$
...
$$\mathcal{J} = \{ s_j \in \mathcal{S} \mid \nabla W(s_j) > \tau \}$$
...
$$R_\mathcal{J}^* = \arg\min_{R_\mathcal{J}} \sum_{f=1}^{F} \left[ \mathcal{L}_{\text{CD}}(\mathcal{J}_c(R_\mathcal{J}), \mathcal{S}_f) + \lambda \, \mathcal{L}_{\text{L2}}(\mathcal{G}_c(R_\mathcal{J}), \mathcal{G}_f) \right]$$
...
$$\mathcal{D}_{\text{src}} = \{\mathcal{P}_l \mid \mathcal{P}_l = (R_\mathcal{J}^l, T_B^l), \; l = 1, \ldots, L\}, \quad L = F - p + 1$$
...
$$\mathcal{P}^{\text{match}} = \arg\min_{\mathcal{P}_l \in \mathcal{D}_{\text{src}}} d_{SO(3)}(R_{\mathcal{J}'}^{\text{novel}}, R_{\mathcal{J}'}^l)$$
...
$$T_{\mathcal{B}'}^{\text{aligned}} = R_{\text{align}} \circ T_{\mathcal{B}'}^{\text{match}}$$
...
$$T_{\mathcal{B}'}^{\ell+1} = (1 - \lambda_\alpha) \, T_{\mathcal{B}'}^{\ell} + \lambda_\alpha \, \bar{T}_{\mathcal{B}'}^{\ell}$$
