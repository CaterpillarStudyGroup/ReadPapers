...
#### Context 的构建 \\(\mathbf{c} _S = (\mathbf{m} _S \quad  \mathrm{o}  \mathbf{z} ,\quad \mathbf{m} _S)\\)     


#### Context 的构建 \\(\begin{matrix}\mathbf{c} _S = & (\mathbf{m} _S & \mathrm{o}  \mathbf{z} , &\mathbf{m} _S) \end{matrix}\\)

1. 构建 mask，长度为 T，需要参考预置内容的帧置为1，需要生成的帧置为0。    
2. 构建Z。训练时取GT对应帧，调整到对应(w,h), 经过SD Encoder,即为Z。推断时，取上一批生成过程的中间结果Z的对应帧。    
3. 构建\\(C_s\\). Z 与 mosk 逐元素乘，其结果在channel维度上与 mask concat. 最终shape为 (B,T,c+1,w,h)      
