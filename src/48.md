...
#### Context 的构建 \\(C_s=(m_s,oz,m_s)\\)     

1. 构建mask，长度为T，需要参考预置内容上需要生成的帧置为0。    
2. 构建Z。训练时取GT对应帧，调整经过SD Encoder,即为Z。推断生成过程的中间结果Z的对应帧。    
3. 构建\\(G_s\\). Z与mosk逐元素乘，其结果上与 mask concat. 最终shape为（B     
