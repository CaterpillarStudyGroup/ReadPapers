...
答：先训一个 text-base 光流生成的基模型，然后以 ControlNet 形式注入控制条件，得到条件控制的光流生成模型。     
> [&#x2753;] 为什么不直接注入控制条件，而是分两步？
...
帧步长 i 的注入方式: MLP(time embdding) + MLP(step 帧步长 i 的注入方式: MLP(time embdding) + MLP(step em)   
)见图1     
...
|--|--|--|--|--|
|ref image emb|noise\\(_1\\)|noise\\(_2\\)|......|noise\\(_h\\)|
...
> &#x2705; reference image 与 noise concat 的作用。   
