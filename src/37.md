...
以以下方式把PTM \\(\Tau \\)引入到UNet attention layer中：离骨架距离远（被认为是背景）&#x2192; D 大 &#x2192;\\(\Tau \\)大 &#x2192; attention score 小 &#x2192; 与时间相关性小 &#x2192; 稳定    

![](./assets/屏幕快照%202024-07-21%2010.44.49.png)

attention map 中的值比较平均，说明Output 相当于input 作者期望背景表现为这种效果。    
attention map 的值比较大，说明在这个位置看比较        
attention map 中比较大的值出现在对角线上，说明相关没看明显规。    
attention map 中没有较大值，说明没有明显相关性。      
attention map 是\\(QK^T\\) 矩阵，在时序层，Q.shape=K.shape=(bxwxh)   
因此，attention map.shape=(bxwxh)xfxf=bxwxh   
