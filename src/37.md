...
|reference image|appearance information za||
|encoder feature z<br>appearance information za|denoised image|UNet Decoder<br>以attention的方式结合z和za<br>在UNet的所有attention层中引入APPA layer（类似LoRA）<br>没有APPA的UNet能生成与driving pose相似的动作，但不是保持reference image中的外观。引入APPA层有以下三个作用：<br>1. 解决frozen ControlNet引入的artifacts<br>2. 防止对training domain过拟合<br>3. appearance与pose解耦|
